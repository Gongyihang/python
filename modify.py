...
if comm_rank == 0:
    ...
    #遗传算法的实体
    ga = GA(DNA_size=N_CITIES, cross_rate=CROSS_RATE1, mutation_rate=MUTATE_RATE1, pop_size=POP_SIZE)
    #环境
    env = TravelSalesPerson(N_CITIES)
elif comm_rank == 2:
    #遗传算法的实体
    ga = GA(DNA_size=N_CITIES, cross_rate=CROSS_RATE2, mutation_rate=MUTATE_RATE2, pop_size=POP_SIZE)
    #环境
    env = TravelSalesPerson(N_CITIES)
elif comm_rank == 3:
    #遗传算法的实体
    ga = GA(DNA_size=N_CITIES, cross_rate=CROSS_RATE3, mutation_rate=MUTATE_RATE3, pop_size=POP_SIZE)
    #环境
    env = TravelSalesPerson(N_CITIES)
else:
    #遗传算法的实体
